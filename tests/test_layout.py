...
def test_gcp_symbols_layout_and_size():
    ...
    # Verify GCP nodes have size 80x115 (wrapped), regular node has 140x60
    assert gcp_node1.width == 80
    assert gcp_node1.height == 115
    assert gcp_node2.width == 80
    assert gcp_node2.height == 115
...
def test_databricks_and_attributes():
    ...
    # Verify standard CSS naming attributes
    css_node = Node("CSS Node", attributes={
        "background-color": "#eeeeee",
        "border-color": "#222222",
        "text-color": "#444444"
    })
    css_container = Container("CSS Box", attributes={
    ...
    node_css_params = renderer.theme.get_node_params(css_node)
    assert node_css_params["fill_color"] == "#eeeeee"
    assert node_css_params["stroke_color"] == "#222222"
    assert node_css_params["text_color"] == "#444444"

    container_css_params = renderer.theme.get_container_params(css_container)
    assert container_css_params["fill_color"] == "#fafafa"
    assert container_css_params["stroke_color"] == "#333333"
    assert container_css_params["text_color"] == "#555555"
    assert container_css_params["stroke_dasharray"] == "2,3"
    assert container_css_params["stroke_width"] == 5.0
...
def test_spacing_and_alignment_layout_only():
    from archDraw.parser import parse_dsl
    
    # 1. Test Spacing/Gap parsing and application (Vertical Container)
    dsl_spacing = """
    architecture "Spacing Test" {
        box "Custom Spacing" {
            spacing: 45
            component "Node A"
            component "Node B"
    ...
    """
    root, connections = parse_dsl(dsl_spacing)
    container = root.children[0]
    
    # Verify spacing is parsed correctly into container.gap
    assert container.gap == 45
    
    # Run layout bounds and coordinates calculation
    renderer = SVGRenderer()
    LayoutEngine.calculate_bounds(root, renderer)
    LayoutEngine.apply_offset(root, dx=0, dy=0)
    
    node_a = container.children[0]
    node_b = container.children[1]
    
    # Check vertical separation gap is exactly container.gap (45)
    expected_gap_y = node_b.y - (node_a.y + node_a.height)
    assert expected_gap_y == 45

    # 2. Test Alignment logic: Center alignment in vertical layout
    dsl_align_center_vert = """
    architecture "Align Center Vert Test" {
        box "Parent" {
            alignment: center
            component "Short" [width="100"]
            component "Very Long Name Component" [width="300"]
        }
    }
    """
    root_acv, _ = parse_dsl(dsl_align_center_vert)
    container_acv = root_acv.children[0]
    
    assert container_acv.alignment == "center"
    LayoutEngine.calculate_bounds(root_acv, renderer)
    LayoutEngine.apply_offset(root_acv, dx=0, dy=0)
    
    short_node = container_acv.children[0]
    long_node = container_acv.children[1]
    
    # The short node should be horizontally centered relative to the long node's width
    # In vertical layout, child.x = bl + (max_child_width - child.width) / 2
    bl, br, bt, bb = renderer.get_container_borders(container_acv)
    max_w = max(short_node.width, long_node.width)
    
    expected_short_x = container_acv.x + bl + (max_w - short_node.width) / 2
    assert abs(short_node.x - expected_short_x) < 1.0

    # 3. Test Alignment logic: Right/Down alignment in vertical layout
    dsl_align_right_vert = """
    architecture "Align Right Vert Test" {
        box "Parent" [align="right"] {
            component "Short" [width="100"]
            component "Very Long Name Component" [width="300"]
        }
    }
    """
    root_arv, _ = parse_dsl(dsl_align_right_vert)
    container_arv = root_arv.children[0]
    
    assert container_arv.alignment == "right"
    LayoutEngine.calculate_bounds(root_arv, renderer)
    LayoutEngine.apply_offset(root_arv, dx=0, dy=0)
    
    short_node_r = container_arv.children[0]
    long_node_r = container_arv.children[1]
    
    # In vertical layout right alignment: child.x = bl + (max_child_width - child.width)
    max_w_r = max(short_node_r.width, long_node_r.width)
    bl_r, _, _, _ = renderer.get_container_borders(container_arv)
    
    expected_short_x_r = container_arv.x + bl_r + (max_w_r - short_node_r.width)
    assert abs(short_node_r.x - expected_short_x_r) < 1.0

    # 4. Test Alignment logic: Center alignment in horizontal layout
    dsl_align_center_horiz = """
    architecture "Align Center Horiz Test" {
        box "Parent" {
            layout: horizontal
            align: center
            component "Short" [height="40"]
            component "Tall Component" [height="150"]
        }
    }
    """
    root_ach, _ = parse_dsl(dsl_align_center_horiz)
    container_ach = root_ach.children[0]
    
    assert container_ach.alignment == "center"
    LayoutEngine.calculate_bounds(root_ach, renderer)
    LayoutEngine.apply_offset(root_ach, dx=0, dy=0)
    
    short_node_h = container_ach.children[0]
    tall_node_h = container_ach.children[1]
    
    # In horizontal layout center alignment: child.y = bt_chosen + (max_child_height - child.height) / 2
    # Let's find bt_chosen for the container
    bl, br, bt_base, bb = renderer.get_container_borders(container_ach)
    title_options = TextRenderer.get_wrapping_options(container_ach.name)
    title_opt_chosen = title_options[0] # defaults to unwrapped title since width is large enough
    bt_chosen = bt_base + (len(title_opt_chosen) - 1) * 15
    
    max_h = max(short_node_h.height, tall_node_h.height)
    expected_short_y = container_ach.y + bt_chosen + (max_h - short_node_h.height) / 2
    assert abs(short_node_h.y - expected_short_y) < 1.0
