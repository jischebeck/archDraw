...
def route_grid(src: Node, tgt: Node, src_pts: dict, tgt_pts: dict, all_elements: list, routed_points=None) -> dict:
    ...
    # Determine vertical connection orientation based on Lowest Common Ancestor (LCA) layout mode
    def get_layout_orientation(s, t):
        path_s = []
        curr = s
        while curr in parents:
            curr = parents[curr]
        ...
        return abs(s_center_y - t_center_y) > abs(s_center_x - t_center_x)

    s_center_x, s_center_y = src_center_x, src_center_y
    t_center_x, t_center_y = tgt_center_x, tgt_center_y
    is_vertical_conn = get_layout_orientation(src, tgt)
