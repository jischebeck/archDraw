...
| `margin` | Containers, Nodes | Spacing outside the element's boundary. Supports standard CSS directional shorthands (e.g. `"10"`, `"10 20"`, `"10 20 30 40"`). | `margin="15px"` |
| `margin-top` / `-bottom` / `-left` / `-right` | Containers, Nodes | Spacing outside the element for a specific side. | `margin-left="30px"` |
| `padding` | Containers | Spacing inside the container's boundary. Supports standard CSS directional shorthands. | `padding="20px 40px"` |
| `padding-top` / `-bottom` / `-left` / `-right` | Containers | Spacing inside the container for a specific side. | `padding-top="15px"` |

> **Note:** When both a shorthand (`color`) and its long form (`border_color`) are specified, the long form takes precedence. Shorthand margin and padding rules also expand to directional properties standard to CSS (e.g. `margin="10 20"` sets top/bottom to 10px and left/right to 20px).
...
## 7. Example: Three-Tier CSS Web Application Architecture

This example showcases the usage of standard CSS property naming conventions (`background-color`, `border-color`, `border-style`, `border-width`, `margin`, and `padding`) to build a beautifully styled three-tier web application architecture:
...
architecture "Three-Tier Web Application" {
    ...
    // Presentation tier box using CSS background, border properties, and padding
    ...
    // Application tier box using explicit margin rules to separate containers
    ...
    // Data tier box with red color scheme mapping
